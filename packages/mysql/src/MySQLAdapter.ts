...
import type { MySQLAdapter as MySQLAdapterInterface } from './Adapter';
import type {
  MySQLColumn,
  MySQLEvent,
  MySQLEventDDL,
 ...
 */
export class MySQLAdapter implements MySQLAdapterInterface {
  ...
  public async getTables(): Promise<Table[]> {
    ...
    const tablesDDL = await Promise.all(
      tables.map(t => {
        return this.query<MySQLTableDDL[]>(
  ...
  public async getEvents(): Promise<Event[]> {
    ...
    const eventsDDL = await Promise.all(
      events.map(e => {
        return this.query<MySQLEventDDL[]>(
  ...
  public async getTriggers(tables: Table[]): Promise<void> {
    ...
    const triggersDDL = await Promise.all(
      triggers.map(t => {
        return this.query<MySQLTriggerDDL[]>(
  ...
  private async getVars(): Promise<void> {
    ...
    const vars = await this.query<Array<MySQLVariable<string>>>(
